...
/**
 * Orchestrate DOM/bootstrap wiring for the game surface. Accepts injected
 * document/window handles to keep tests deterministic while the production
 * entrypoint uses browser globals.
 *
 * @param {Object} [options]
 * @param {Document} [options.documentRef=document] document handle to attach
 * lifecycle listeners to.
 * @param {Window} [options.windowRef=window] optional window-like object for
 * dependency discovery.
 */
export function bootstrapGame({ documentRef = document, windowRef = window } = {}) {
...
const { Hex, Layout, SQRT3 } = resolveHexGrid({ hexGridModule: HexGrid, windowObj: window });

const { CAMERA_MOTION_CONFIG, AMBIENCE_CONFIG } = resolveRenderConfig({
    renderConfigModule: RenderConfig,
    windowObj: window
});
